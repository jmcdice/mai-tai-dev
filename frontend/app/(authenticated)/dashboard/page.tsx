  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4 py-2">
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-lg border px-4 py-3 text-sm font-medium transition ${
                workspaceType === 'chat'
                  ...
                  : 'border-border-strong bg-surface2/50 text-muted-foreground hover:border-border-strong hover:text-muted-foreground'
              }`}
            >
          ...
          {workspaceType === 'agent' && (
            <>
              ...
              {/* Purpose */}
              <div>
                <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Purpose</label>
                <textarea
                  placeholder="What should this agent do? (e.g. Search for Rivian R1S deals daily and report findings)"
                  value={agentPurpose}
                  onChange={(e) => setAgentPurpose(e.target.value)}
                  rows={3}
                  className="w-full resize-none rounded-lg border border-border-strong bg-surface2/80 px-4 py-3 text-foreground placeholder-muted-foreground transition focus:border-primary focus:outline-none focus:ring-1 focus:ring-ring"
                />
              </div>
              ...
              {agentTemplate === 'coder' && (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Repository URL</label>
                  <input
                    type="text"
      ...
      {isLoading && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {[...Array(4)].map((_, i) => (
              <div key={`stat-skeleton-${i}`} className="h-24 animate-pulse rounded-xl bg-card" />
            ))}
          </div>
          {/* Heatmap skeleton */}
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {[...Array(6)].map((_, i) => (
              <div key={`project-skeleton-${i}`} className="aspect-[4/3] animate-pulse rounded-xl bg-card" />
            ))}
          </div>
        </div>
      )}
      ...
      {!isLoading && workspaces.length === 0 && (
        <div className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary/10 via-accent2/10 to-accent2/10 p-8">
          <div className="flex flex-col items-center text-center md:flex-row md:text-left md:items-start md:gap-6">
            <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary/20 md:mb-0">
              <RocketLaunchIcon className="h-8 w-8 text-primary" />
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-bold text-foreground">🍹 Welcome to Mai-Tai!</h3>
        ...
        <>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-muted-foreground">Your activity overview</p>
            </div>
            <Button buttonType="primary" onClick={() => setIsDialogOpen(true)}>
              <RocketLaunchIcon className="mr-2 h-5 w-5" />
              Add Workspace
          ...
          <div>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">Most Active Workspaces</h2>
              <Link
                href="/workspaces"
...
function StatCard({
  ...
  const colorClasses = {
    ...
    gray: 'bg-surface2/20 text-muted-foreground',
  };
